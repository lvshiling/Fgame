...
crossStart(){
        INDEX=$1
        EXEC=./cross_$suffix$INDEX
        PIDFILE=./cross_$INDEX.pid
        CONF=./config/cross_$INDEX.json
        cd $WORKPATH/cross_$INDEX

        if [ -f $PIDFILE ]
        then
                echo "$PIDFILE 已经存在,全平台服($INDEX)已经启动或奔溃"
        else
                ...
                if [ -x /proc/${PID} ]
                then
                        echo "全平台服($INDEX)启动成功"
                        echo $PID>$PIDFILE
                else
                        echo "全平台服($INDEX)启动失败"
                fi       
        fi           
}
...
crossStop(){
        ...
        if [ ! -f $PIDFILE ]
        then
                echo "$PIDFILE 不存在,全平台服($INDEX)已经关闭"
        else
                PID=$(cat $PIDFILE)
                echo "正在停止全平台服($INDEX)"
                kill $PID
...
cross(){
        ...
        status)
                ...
                then
                        ...
                        if [ -x /proc/${PID} ]
                        then
                                echo "全平台服($INDEX)正在运行"
                        else 
                                echo "全平台服($INDEX)已经停止"
                        fi
                else
                        echo "全平台服($INDEX)已经停止"
                fi
                ;;
        update)
                cp ../cross ./cross_$suffix$INDEX
                echo "全平台服($INDEX)更新成功"
                ;;
        upgrade)
                crossStop $INDEX
                cp ../cross ./cross_$suffix$INDEX
        ...
        cross)
                case "$2" in 
                        ...
                        *)
                                ...
                                then
                                        # 查找游戏服
                                        for crossDir in `ls $WORKPATH`
                                        do
                                                if [[ -d "$WORKPATH/$crossDir" && $crossDir =~ cross_[0-9]+ ]]
                                                then
                                                index=$(echo $crossDir | cut -d "_" -f2)
                                                echo "全平台目录$crossDir,索引$index"
                                                serverList+=("$index")
                                                fi
                                        done
