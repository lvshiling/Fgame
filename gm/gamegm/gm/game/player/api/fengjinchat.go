...
func handleForbidChat(rw http.ResponseWriter, req *http.Request) {
	form := &forbidChatRequest{}
	...
	info := &userremotemodel.ForbidPlayerChat{
	...
	err = service.ForbidPlayerChat(int32(serverid), info)
	if err != nil {
		log.WithFields(log.Fields{
			"error":    err,
			"serverid": serverid,
