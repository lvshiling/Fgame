...
type unForbidChatRequest struct {
	PlatformId int32  `json:"centerPlatformId"`
	ServerId   int32  `json:"centerServerId"`
	PlayerId   string `json:"playerId"`
}

func handleUnForbidChat(rw http.ResponseWriter, req *http.Request) {
	form := &unForbidChatRequest{}
	...
	err = service.UnForbidPlayerChat(int32(serverid), changeStringToInt64(form.PlayerId))
	if err != nil {
		log.WithFields(log.Fields{
			"error":    err,
			"serverid": serverid,
