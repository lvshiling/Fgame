...
import { getGameFeedBackFeeList } from "@/api/feedbackfee";
import { feedBackGameStatus } from "@/types/feedback";
...
export default {
  ...
  filters: {
    ...
    parseOrderStatus : function(value){
        if(feedBackGameStatus[value]){
            return feedBackGameStatus[value].name
        }
        return ""
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      getGameFeedBackFeeList(this.listQuery)
        .then(res => {
