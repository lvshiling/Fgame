<template>
 <div class="app-container">
    ...
    <el-table
            ...
            <el-table-column label="接收者id" width="80px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row.recvId}}</span>
                </template>
            </el-table-column>
            <el-table-column label="接收者名字" width="120px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row.recvName}}</span>
                </template>
            </el-table-column>
            <el-table-column label="消息类型" width="80px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row.msgType}}</span>
                </template>
            </el-table-column>
            <el-table-column label="内容" min-width="150px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row.content}}</span>
                </template>
            </el-table-column> -->

            <el-table-column v-for="(item,index) in metaColumnArray" :label="item.lab" v-if="index < metaColumnArray.length-1" width="100px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row[item.key] | commonFilter(item.showType)}}</span>
                </template>
            </el-table-column>
            <el-table-column v-for="(item,index) in metaColumnArray" :label="item.lab" v-if="index == metaColumnArray.length-1" min-width="100px" align="left">
                <template slot-scope="scope">
                    <span>{{ scope.row[item.key] | commonFilter(item.showType)}}</span>
                </template>
            </el-table-column>
...
import { binaryToStr } from "@/utils/binary";
export default {
  name: "PlayerLogList",
  directives: {
    waves
