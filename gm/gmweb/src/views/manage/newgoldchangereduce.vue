<template>
    <div class="app-container">
        ...
        <el-table
            ...
            <el-table-column fixed="left" label="增加原因" align="center" width="120px" >
            ...
            <el-table-column  v-for="(item,index) in queryServerList" :label="item.serverName"  align="center" width="300px" v-bind:key="index" >
                <el-table-column label="增加数量" align="center" width="140px" >
                    <template slot-scope="scope">
                        <span>{{ scope.row.serverMap[item.serverId] ? scope.row.serverMap[item.serverId].changeNum : "" }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="增加占比" align="center" width="80px" >
                    <template slot-scope="scope">
                        <span>{{ scope.row.serverMap[item.serverId] ? (scope.row.serverMap[item.serverId].changeNum*100/item.changeNum).toFixed(2) +"%" : "" }}</span>
                    </template>
                </el-table-column>
...
import { getNewGoldChange, getGoldChangeType } from "@/api/goldchange";

export default {
  name: "NewGoldChangeList",
  directives: {
    waves
  },
  filters: {
  ...
  methods: {
    ...
    getList() {
      ...
      getNewGoldChange(this.listQuery)
        .then(res => {
