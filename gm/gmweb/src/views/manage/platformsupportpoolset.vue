<template>
    <div class="app-container">
        ...
        <el-table
            ...
            <el-table-column label="设置Id" align="center" width="180px">
                <template slot-scope="scope">
                    <span>{{ scope.row.id }}</span>
                </template>
            </el-table-column>
            <el-table-column label="中心平台Id" align="center" width="100px">
                <template slot-scope="scope">
                    <span>{{ scope.row.centerPlatformId }}</span>
                </template>
            </el-table-column>
            <el-table-column label="中心平台名称" align="center" width="180px">
                <template slot-scope="scope">
                    <span>{{ getCenterPlatFormName(scope.row.centerPlatformId) }}</span>
                </template>
            </el-table-column>
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="120px" style="width: 600px; margin-left:50px;">
                <el-form-item v-if="dialogStatus=='create'" label="中心平台">
                ...
                <el-form-item v-if="dialogStatus=='update'" label="中心平台">
                    <el-input :value ="getCenterPlatFormName(temp.centerPlatformId)" :disabled="true"/>
                </el-form-item>

                <el-form-item label="当前元宝">
                    <el-input type="number" v-model="temp.gold"/>
                </el-form-item>
...
import {
  addPlatformSupportPoolSet,
  getPlatformSupportPoolList,
  updatePlatformSupportPoolSet,
  deletePlatformSupportPoolSet
} from "@/api/supportpool";
export default {
  name: "PlatformSupportPool",
  directives: {
    waves
  },
  created() {
  ...
  methods: {
    ...
    createData(e) {
      addPlatformSupportPoolSet(this.temp).then(res => {
    ...
    updateData(e) {
      updatePlatformSupportPoolSet(this.temp).then(res => {
        this.dialogFormVisible = false;
        this.showSuccess();
        this.loadData();
      });
    ...
    deleteData(e) {
      deletePlatformSupportPoolSet(this.temp).then(res => {
    ...
    loadData() {
      ...
      getPlatformSupportPoolList(this.listQuery).then(res => {
    ...
    findPlatFormItemByCenter(centerPlatformId) {
      let platform = this.platformList.find(n => {
    ...
    getCenterPlatFormName(centerPlatformId){
        let item = this.findPlatFormItemByCenter(centerPlatformId);
        if(item){
