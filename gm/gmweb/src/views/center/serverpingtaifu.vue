    ...
    <div class="app-container">
        ...
        <el-row :gutter="20">
            <el-col :span="12">
                ...
                <el-table
                    ...
                    <!-- <el-table-column label="已绑交易战区序号" width="140px" align="left">
                        <template slot-scope="scope">
                        <span>{{ scope.row.pingTaiFuServerId }}</span>
                        </template>
                    </el-table-column> -->
            ...
            <el-col :span="12">
                ...
                <div>已添加,全平台服务ID:{{item.serverId}},名称:{{item.serverName}}</div>   
                    <el-table
                        v-loading="listLoading"
                        :key="tableKeyEn"
        ...
        <el-dialog title="绑定全平台" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                ...
                <el-form-item label="全平台">
                    <el-select v-model="temp.pingTaiFuServerId" class="filter-item" placeholder="全平台">
        ...
        <el-dialog title="批量绑定全平台" :visible.sync="dialogMupliFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                <el-form-item label="全平台">
                    <el-select v-model="temp.pingTaiFuServerId" class="filter-item" placeholder="全平台">
                        <el-option v-for="citem in quanPingTaiServerList" :key="citem.serverId" :label="citem.serverId" :value="citem.serverId"/>
...
import {
  ...
  updateCenterServerPingTaiFu,
  updateCenterServerPingTaiFuArray,
  getCenterServerListQuanPingTai
...
export default {
  ...
  methods: {
    ...
    loadData() {
      let that = this;
      getCenterServerListQuanPingTai().then(res => {
    ...
    updateBind(e) {
      let id = this.temp.id;
      let parentServerId = this.temp.pingTaiFuServerId;
      updateCenterServerPingTaiFu(id, parentServerId).then(res => {
    ...
    updateBindArray(e){
      ...
      updateCenterServerPingTaiFuArray(id, parentServerId).then(res => {
    ...
    deleteData(e) {
      let id = this.temp.id;
      updateCenterServerPingTaiFu(id, 0).then(res => {
        this.showSuccess();
        this.loadData();
