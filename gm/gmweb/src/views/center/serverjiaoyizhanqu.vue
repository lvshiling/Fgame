<template>
    <div class="app-container">
        ...
        <el-row :gutter="20">
            <el-col :span="12">
                ...
                <el-table
                    ...
                    <!-- <el-table-column label="已绑交易战区序号" width="140px" align="left">
                        <template slot-scope="scope">
                        <span>{{ scope.row.jiaoYiZhanQuServerId }}</span>
                        </template>
                    </el-table-column> -->
            ...
            <el-col :span="12">
                <div v-for="(item,index) in jiaoYiZhanQuServerList" :key="index">
                <div>已添加,交易战区服务ID:{{item.serverId}},名称:{{item.zhanquName}}</div>   
                    <el-table
                        v-loading="listLoading"
                        :key="tableKeyEn"
        ...
        <el-dialog title="绑定交易战区" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                ...
                <el-form-item label="交易战区">
                    <el-select v-model="temp.jiaoYiZhanQuServerId" class="filter-item" placeholder="交易战区">
                        <el-option v-for="citem in jiaoYiZhanQuServerList" :key="citem.serverId" :label="citem.serverId" :value="citem.serverId"/>
                    </el-select>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="批量绑定交易战区" :visible.sync="dialogMupliFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                <el-form-item label="交易战区">
                    <el-select v-model="temp.jiaoYiZhanQuServerId" class="filter-item" placeholder="交易战区">
                        <el-option v-for="citem in jiaoYiZhanQuServerList" :key="citem.serverId" :label="citem.serverId" :value="citem.serverId"/>
...
import {
  ...
  updateCenterServerJiaoYiZhanQu,
  updateCenterServerJiaoYiZhanQuArray
} from "@/api/centerserver";
import { getJiaoYiZhanQuListAll } from "@/api/jiaoyizhanqu";
...
export default {
  ...
  data() {
    return {
      ...
      temp: {
        ...
        platformId: undefined,
        serverId: undefined,
        jiaoYiZhanQuServerId: undefined
      },
      multipleSelection: []
    };
  },
  ...
  methods: {
    ...
    loadData() {
      ...
      let that = this;
      getJiaoYiZhanQuListAll(this.listQuery).then(res => {
        this.jiaoYiZhanQuServerList = res.itemArray;
        getCenterServerListByServerType(this.listQuery.platformId, 0).then(res => {
          ...
          if (itemArray.length > 0) {
            for (let i = 0; i < itemArray.length; i++) {
              ...
              if (item.jiaoYiZhanQuServerId == 0) {
                that.unList.push(item);
                continue;
              }
              for (let j = 0; j < that.jiaoYiZhanQuServerList.length; j++) {
                let jiaoYiItem = that.jiaoYiZhanQuServerList[j];
                if (jiaoYiItem.serverId == item.jiaoYiZhanQuServerId) {
    ...
    updateBind(e) {
      let id = this.temp.id;
      let parentServerId = this.temp.jiaoYiZhanQuServerId;
      updateCenterServerJiaoYiZhanQu(id, parentServerId).then(res => {
    ...
    updateBindArray(e){
      ...
      updateCenterServerJiaoYiZhanQuArray(id, parentServerId).then(res => {
    ...
    deleteData(e) {
      let id = this.temp.id;
      updateCenterServerJiaoYiZhanQu(id, 0).then(res => {
        this.showSuccess();
        this.loadData();
