<template>
    <div class="app-container">
        ...
        <el-row :gutter="20">
            ...
            <el-col :span="12">
                <div v-for="(item,index) in zhanQuServerList" :key="index">
                <div>已添加,战区服务ID:{{item.serverId}},名称:{{item.serverName}}</div>   
                    <el-table
                        v-loading="listLoading"
                        :key="tableKeyEn"
        ...
        <el-dialog title="绑定战区" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                ...
                <el-form-item label="战区">
                    <el-select v-model="temp.parentServerId" class="filter-item" placeholder="战区">
                        <el-option v-for="citem in zhanQuServerList" :key="citem.serverId" :label="citem.serverId" :value="citem.serverId"/>
                    </el-select>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="批量绑定战区" :visible.sync="dialogMupliFormVisible">
            <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
                <el-form-item label="战区">
                    <el-select v-model="temp.parentServerId" class="filter-item" placeholder="战区">
                        <el-option v-for="citem in zhanQuServerList" :key="citem.serverId" :label="citem.serverId" :value="citem.serverId"/>
...
import {
  ...
  getCenterServerListZhanQuList,
  updateCenterServerParentId,
  updateCenterServerParentIdArray
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      let that = this;
      getCenterServerListZhanQuList(this.listQuery.platformId).then(res => {
        this.zhanQuServerList = res.itemArray;
        getCenterServerListByServerType(this.listQuery.platformId, 0).then(res => {
          ...
          if (itemArray.length > 0) {
            for (let i = 0; i < itemArray.length; i++) {
              ...
              if (item.parentServerId == 0) {
                that.unList.push(item);
                continue;
              }
              for (let j = 0; j < that.zhanQuServerList.length; j++) {
                let jiaoYiItem = that.zhanQuServerList[j];
                if (jiaoYiItem.serverId == item.parentServerId) {
    ...
    updateBind(e) {
      let id = this.temp.id;
      let parentServerId = this.temp.parentServerId;
    ...
    updateBindArray(e){
      ...
      updateCenterServerParentIdArray(id, parentServerId).then(res => {
    ...
    deleteData(e) {
      let id = this.temp.id;
      updateCenterServerParentId(id, 0).then(res => {
        this.showSuccess();
        this.loadData();
