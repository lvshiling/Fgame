...
func init() {
	playerpropertyproperty.RegisterPlayerPropertyEffector(playerpropertytypes.PlayerPropertyEffectorTypeLingTongFaBao, LingTongFaBaoPropertyEffect)
}

//灵宝作用器
func LingTongFaBaoPropertyEffect(p player.Player, prop *propertycommon.SystemPropertySegment) {
	if !p.IsFuncOpen(funcopentypes.FuncOpenTypeLingTongFaBao) {
		return
	}
	...
	//附加系统属性
	additionSysPropertyEffect(p, additionsystypes.AdditionSysTypeLingTongFaBao, prop)
	skillByModulePropertyEffect(p, skilltypes.SkillFirstTypeLingTongFaBaoSystemSkill, prop)
}
