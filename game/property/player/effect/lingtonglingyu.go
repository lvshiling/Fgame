...
func init() {
	playerpropertyproperty.RegisterPlayerPropertyEffector(playerpropertytypes.PlayerPropertyEffectorTypeLingTongLingYu, LingTongLingYuPropertyEffect)
}

//灵域作用器
func LingTongLingYuPropertyEffect(p player.Player, prop *propertycommon.SystemPropertySegment) {
	if !p.IsFuncOpen(funcopentypes.FuncOpenTypeLingTongLingYu) {
		return
	}
	...
	//附加系统属性
	additionSysPropertyEffect(p, additionsystypes.AdditionSysTypeLingTongLingYu, prop)
	skillByModulePropertyEffect(p, skilltypes.SkillFirstTypeLingTongLingYuSystemSkill, prop)
}
