...
func init() {
	playerpropertyproperty.RegisterPlayerPropertyEffector(playerpropertytypes.PlayerPropertyEffectorTypeLingTongWing, LingTongWingPropertyEffect)
}

//灵翼作用器
func LingTongWingPropertyEffect(p player.Player, prop *propertycommon.SystemPropertySegment) {
	if !p.IsFuncOpen(funcopentypes.FuncOpenTypeLingTongWing) {
		return
	}
	...
	//附加系统属性
	additionSysPropertyEffect(p, additionsystypes.AdditionSysTypeLingTongWing, prop)
	skillByModulePropertyEffect(p, skilltypes.SkillFirstTypeLingTongWingSystemSkill, prop)
}
