...
func init() {
	playerpropertyproperty.RegisterPlayerPropertyEffector(playerpropertytypes.PlayerPropertyEffectorTypeLingTongShenFa, LingTongShenFaPropertyEffect)
}

//灵身作用器
func LingTongShenFaPropertyEffect(p player.Player, prop *propertycommon.SystemPropertySegment) {
	if !p.IsFuncOpen(funcopentypes.FuncOpenTypeLingTongShenFa) {
		return
	}
	...
	//附加系统属性
	additionSysPropertyEffect(p, additionsystypes.AdditionSysTypeLingTongShenFa, prop)
	skillByModulePropertyEffect(p, skilltypes.SkillFirstTypeLingTongShenFaSystemSkill, prop)
}
