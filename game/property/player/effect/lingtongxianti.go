...
func init() {
	playerpropertyproperty.RegisterPlayerPropertyEffector(playerpropertytypes.PlayerPropertyEffectorTypeLingTongXianTi, LingTongXianTiPropertyEffect)
}

//灵体作用器
func LingTongXianTiPropertyEffect(p player.Player, prop *propertycommon.SystemPropertySegment) {
	if !p.IsFuncOpen(funcopentypes.FuncOpenTypeLingTongXianTi) {
		return
	}
	...
	//附加系统属性
	additionSysPropertyEffect(p, additionsystypes.AdditionSysTypeLingTongXianTi, prop)
	skillByModulePropertyEffect(p, skilltypes.SkillFirstTypeLingTongXianTiSystemSkill, prop)
}
