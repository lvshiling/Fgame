...
func init() {
	playerinventory.RegisterUseHandler(itemtypes.ItemTypeUpLevelDan, itemtypes.ItemUpLevelDanSubTypeSuperUpLevelDan, playerinventory.ItemUseHandleFunc(handleSuperUplevelDan))
}

func handleSuperUplevelDan(pl player.Player, it *playerinventory.PlayerItemObject, num int32, chooseIndexList []int32, args string) (flag bool, err error) {
	if num <= 0 {
		return
	}
	itemId := it.ItemId
	...
	reason := commonlog.LevelLogReasonEatSuperUplevelDan
