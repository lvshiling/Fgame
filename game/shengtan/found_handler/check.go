...
func init() {
	found.RegistFoundCheckHandler(foundtypes.FoundResourceTypeShengTan, found.FoundCheckHandlerFunc(checkFoundBack))
}
...
func checkFoundBack(pl player.Player) bool {
	...
	timeTemp := activitytemplate.GetActivityTemplateService().GetActiveByType(activitytypes.ActivityTypeAllianceShengTan).GetOnDateTimeTemplate(preDay, openTime, mergeTime)
	if timeTemp == nil {
		return false
	}
