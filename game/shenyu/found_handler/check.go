...
func init() {
	found.RegistFoundCheckHandler(foundtypes.FoundResourceTypeShenYu, found.FoundCheckHandlerFunc(checkFoundBack))
}
...
func checkFoundBack(pl player.Player) bool {
	...
	timeTemp := activitytemplate.GetActivityTemplateService().GetActiveByType(activitytypes.ActivityTypeShenYu).GetOnDateTimeTemplate(preDay, openTime, mergeTime)
	if timeTemp == nil {
		return false
	}
	return true
