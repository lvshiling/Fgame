...
//队伍竞技停止匹配
func teamArenaStopMatch(target event.EventTarget, data event.EventData) (err error) {
	pl := target.(player.Player)
	teamObj := data.(*team.TeamObject)
	//发送匹配
	captainMem := teamObj.GetCaptain()
	//推送用户
	arenalogic.OnTeamArenaStopMatch(pl, teamObj)
...
func init() {
	gameevent.AddEventListener(teameventtypes.EventTypeTeamArenaStopMatch, event.EventListenerFunc(teamArenaStopMatch))
}
