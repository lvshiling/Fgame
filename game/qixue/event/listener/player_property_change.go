...
func playerPropertyChange(target event.EventTarget, data event.EventData) (err error) {
	...
	if propertyEffectorType != playerpropertytypes.PlayerPropertyEffectorTypeQiXue {
		return
	}
	...
	power := propertyManager.GetModuleForce(playerpropertytypes.PlayerPropertyEffectorTypeQiXue)
	qixueManager.QiXuePower(power)
