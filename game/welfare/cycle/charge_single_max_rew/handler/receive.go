...
func init() {
	welfare.RegisterReceiveHandler(welfaretypes.OpenActivityTypeCycleCharge, welfaretypes.OpenActivityCycleChargeSubTypeSingleChargeMaxRew, welfare.ReceiveHandlerFunc(cycleSingleMaxRewReceive))
}

//每日单笔充值领取奖励请求逻辑
func cycleSingleMaxRewReceive(pl player.Player, rewId int32) (err error) {
		...
		if !info.IsCanReceiveRewards(canRewGold) {
			log.WithFields(
				log.Fields{
					...
					"receiveRewRecord": info.ReceiveRewRecord,
