...
func init() {
	processor.Register(codec.MessageType(uipb.MessageType_CS_OPEN_ACTIVITY_CYCLE_SINGLE_CHARGE_REWARDS_TYPE), dispatch.HandlerFunc(handlerCycleSingleReceive))
}

//处理领取每日单笔充值
func handlerCycleSingleReceive(s session.Session, msg interface{}) (err error) {
	log.Debug("welfare:处理每日单笔充值领取奖励请求")

	pl := gamesession.SessionInContext(s.Context()).Player()
	tpl := pl.(player.Player)
	csMsg := msg.(*uipb.CSOpenActivityCycleSingleChargeRewards)
	rewId := csMsg.GetRewId()

	err = receiveCycleSingle(tpl, rewId)
	if err != nil {
		log.WithFields(
			log.Fields{
				"playerId": tpl.GetId(),
				"err":      err,
			}).Error("welfare:处理每日单笔充值领取奖励请求，错误")
		return
	}
	...
	log.WithFields(
		log.Fields{
			"playerId": tpl.GetId(),
		}).Debug("welfare:处理每日单笔充值领取奖励请求完成")

	return
}

//每日单笔充值领取奖励请求逻辑
func receiveCycleSingle(pl player.Player, rewId int32) (err error) {
	...
	scMsg := pbutil.BuildSCOpenActivityCycleSingleChargeRewards(rewId, info.RewRecord, totalRewData, rewItemMap)
	pl.SendMsg(scMsg)
