...
//返利-房产投资
func playerChargeFeedbackHouseInvest(target event.EventTarget, data event.EventData) (err error) {
	...
	for _, timeTemp := range timeTempList {
		...
		// info.CurDayChargeNum += goldNum
		// welfareManager.UpdateObj(obj)

		scMsg := pbutil.BuildSCOpenActivityFeedbackHouseInvestNotice(groupId, info.ChargeNum, info.CurDayChargeNum)
		pl.SendMsg(scMsg)
...
func init() {
	gameevent.AddEventListener(chargeeventtypes.ChargeEventTypeChargeGold, event.EventListenerFunc(playerChargeFeedbackHouseInvest))
}
