...
func (info *LuckyCostDrewInfo) CountLeftTimes(convertLimit, convertRate, minCycle int32) {
	if convertRate == 0 {
		return
	}
...
func (info *LuckyCostDrewInfo) ResetRule() {
	info.Ratio = 0
	info.CycleCount = 0
}

func (info *LuckyCostDrewInfo) IsRuleCD(ruleCycle int32) bool {
	return info.CycleCount <= ruleCycle
}

func init() {
	playerwelfare.RegisterOpenActivityData(welfaretypes.OpenActivityTypeMergeDrew, welfaretypes.OpenActivityDrewSubTypeCostDrew, (*LuckyCostDrewInfo)(nil))
}
