...
func init() {
	processor.Register(codec.MessageType(uipb.MessageType_CS_OPEN_ACTIVITY_ADVANCED_REW_EXTENDED_RECEIVE_TYPE), dispatch.HandlerFunc(handlerAdvancedRewExtendedReceive))
}

//处理领取升阶奖励(随功能开启)
func handlerAdvancedRewExtendedReceive(s session.Session, msg interface{}) (err error) {
	log.Debug("welfare:处理升阶奖励领取奖励请求")

	pl := gamesession.SessionInContext(s.Context()).Player()
	tpl := pl.(player.Player)
	csMsg := msg.(*uipb.CSOpenActivityAdvancedRewExtendedReceive)
	rewId := csMsg.GetRewId()

	err = receiveAdvancedRewExtended(tpl, rewId)
	if err != nil {
		log.WithFields(
			log.Fields{
				"playerId": tpl.GetId(),
...
//升阶奖励领取奖励请求逻辑
func receiveAdvancedRewExtended(pl player.Player, rewId int32) (err error) {
	...
	totalRewExtendedData, rewItemMap, flag := welfarelogic.AddOpenActivityRewards(pl, openTemp)
	if !flag {
		return
	}
	...
	if len(itemNameLinkStr) > 0 {
		...
		advancedNum := chatlogic.FormatModuleNameNoticeStr(fmt.Sprintf(lang.GetLangService().ReadLang(lang.OpenActivityEmailCommonSystemLevelString), info.AdvancedNum))
		...
		content := fmt.Sprintf(lang.GetLangService().ReadLang(lang.OpenActivityAdvancedRewExtendedNotice), plName, acName, moduleName, advancedNum, itemNameLinkStr, link)
	...
	scMsg := pbutil.BuildSCOpenActivityAdvancedRewExtendedReceive(totalRewExtendedData, rewItemMap, rewId, info.RewRecord)
	pl.SendMsg(scMsg)
