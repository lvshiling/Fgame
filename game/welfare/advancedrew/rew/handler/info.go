...
func init() {
	processor.Register(codec.MessageType(uipb.MessageType_CS_OPEN_ACTIVITY_ADVANCED_REW_INFO_TYPE), dispatch.HandlerFunc(handlerAdvancedRewInfo))
}

//处理获取升阶奖励信息
func handlerAdvancedRewInfo(s session.Session, msg interface{}) (err error) {
	log.Debug("welfare:处理获取升阶奖励请求")

	pl := gamesession.SessionInContext(s.Context()).Player()
	tpl := pl.(player.Player)
	csMsg := msg.(*uipb.CSOpenActivityAdvancedRewInfo)
	groupId := csMsg.GetGroupId()

	err = getAdvancedRewInfo(tpl, groupId)
	if err != nil {
		log.WithFields(
			log.Fields{
				"playerId": tpl.GetId(),
...
//获取升阶奖励请求逻辑
func getAdvancedRewInfo(pl player.Player, groupId int32) (err error) {
	...
	scMsg := pbutil.BuildSCOpenActivityAdvancedRewInfo(obj, groupId, startTime, endTime)
	pl.SendMsg(scMsg)
	return
}
