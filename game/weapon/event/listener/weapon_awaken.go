...
//玩家兵魂觉醒
func playerWeaponAwaken(target event.EventTarget, data event.EventData) (err error) {
	...
	attrTemp := weaponTemplate.GetAwakenAttrTemplate()
	if attrTemp == nil {
		return
	}
	...
	content := fmt.Sprintf(lang.GetLangService().ReadLang(lang.WeaponAwakenNotice), playerName, weaponName, powerStr)
...
func init() {
	gameevent.AddEventListener(weaponeventtypes.EventTypeWeaponAwaken, event.EventListenerFunc(playerWeaponAwaken))
}
