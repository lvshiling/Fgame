...
func init() {
	command.Register(gmcommandtypes.CommandTypeEquipBaoKuAttendPoints, command.CommandHandlerFunc(handleSetEquipBaoKuAttendPoints))
}

func handleSetEquipBaoKuAttendPoints(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
	...
	manager.GMSetAttendPoints(int32(num), baoKuTyp)
	...
	// scEquipBaoKuInfoGet := pbutil.BuildSCEquipBaoKuInfoGet(obj, logList, shopBuyCountMap)
