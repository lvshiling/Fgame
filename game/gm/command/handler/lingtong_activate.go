...
func init() {
	command.Register(gmcommandtypes.CommandTypeLingTongActivate, command.CommandHandlerFunc(handleLingTongActive))

}

func handleLingTongActive(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 0 {
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"error":      err,
			}).Warn("gm:处理设置灵童激活,lingTongId不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	if tempTemplateObject == nil {
		log.WithFields(
			log.Fields{
				...
				"lingTongId": lingTongId,
				"error":      err,
			}).Warn("gm:处理设置灵童激活,lingTongId模板不存在")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		return
	}
