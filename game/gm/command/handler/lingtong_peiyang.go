...
func init() {
	command.Register(gmcommandtypes.CommandTypeLingTongPeiYang, command.CommandHandlerFunc(handleLingTongPeiYang))

}

func handleLingTongPeiYang(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"levelStr":   levelStr,
				"error":      err,
			}).Warn("gm:处理设置灵童培养等级,lingTongId不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	if tempTemplateObject == nil {
		log.WithFields(
			log.Fields{
				...
				"lingTongId": lingTongId,
				"error":      err,
			}).Warn("gm:处理设置灵童培养等级,lingTongId模板不存在")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		return
	}

	lingTongPeiYangTemplate := tempTemplateObject.GetLingTongPeiYangByLevel(int32(level))
	...
	flag = manager.GmLingTongPeiYang(int32(lingTongId), int32(level))
	if !flag {
		return
	}
	lingtonglogic.LingTongPropertyChanged(pl)
	...
	scLingTongPeiYang := pbutil.BuildSCLingTongPeiYang(int32(lingTongId), lingTongInfo.GetPeiYangLevel(), lingTongInfo.GetPeiYangPro())
