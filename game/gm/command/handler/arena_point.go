...
func init() {
	command.Register(gmcommandtypes.CommandTypeArenaPoint, command.CommandHandlerFunc(handleArenaPoint))
}

func handleArenaPoint(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) < 1 {
	...
	manager.GMSetPoint(int32(point))
	return
}
