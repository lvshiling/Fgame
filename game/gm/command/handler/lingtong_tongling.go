...
func init() {
	command.Register(gmcommandtypes.CommandTypeLingTongTongLing, command.CommandHandlerFunc(handleLingTongTongLing))

}

func handleLingTongTongLing(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"error":          err,
			}).Warn("gm:处理设置灵童养成类通灵等级,classTypeInt64不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"error":          err,
			}).Warn("gm:处理设置灵童养成类通灵等级,unrealDanLevel不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	lingTongDevTongLingTemplate := lingtongdevtemplate.GetLingTongDevTemplateService().GetLingTongDevTongLingTemplate(classType, int32(unrealDanLevel))
	...
	if lingTongDevTongLingTemplate == nil {
		log.WithFields(
			log.Fields{
				...
				"unrealDanLevel": unrealDanLevel,
				"error":          err,
			}).Warn("gm:处理设置灵童养成类通灵等级,unrealDanLevel模板不存在")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		return
	}
	...
	manager.GmSetLingTongDevTongLing(classType, int32(unrealDanLevel))

	//同步属性
	lingtongdevlogic.LingTongDevPropertyChanged(pl, classType)

	scLingTongDevTongLing := pbutil.BuildSCLingTongDevTongLing(int32(classType), int32(unrealDanLevel), 0)
