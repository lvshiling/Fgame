...
func init() {
	command.Register(gmcommandtypes.CommandTypeAdditionSysShengJi, command.CommandHandlerFunc(handleAdditionSysShengJi))
}

func handleAdditionSysShengJi(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) < 2 {
	...
	tempTemplatet := additionsystemplate.GetAdditionSysTemplateService().GetShengJiByArg(sysType, int32(lev))
	...
	flag := manager.GmSetShengJi(sysType, tempTemplatet.GetLevel())
	if !flag {
		log.WithFields(
			log.Fields{
				"id":  pl.GetId(),
	...
	scAdditionSysShengJi := pbutil.BuildSCAdditionSysShengJi(int32(sysType), int32(lev), 0, int32(res))
