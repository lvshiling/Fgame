...
func init() {
	command.Register(gmcommandtypes.CommandTypeLingTongCaoLiao, command.CommandHandlerFunc(handleLingTongCaoLiao))

}

func handleLingTongCaoLiao(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"error":          err,
			}).Warn("gm:处理设置灵童养成类食草料等级,classTypeInt64不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"error":          err,
			}).Warn("gm:处理设置灵童养成类食草料等级,unrealDanLevel不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	lingTongDevPeiYangTemplate := lingtongdevtemplate.GetLingTongDevTemplateService().GetLingTongDevPeiYangTemplate(classType, int32(unrealDanLevel))
	...
	if lingTongDevPeiYangTemplate == nil {
		log.WithFields(
			log.Fields{
				...
				"unrealDanLevel": unrealDanLevel,
				"error":          err,
			}).Warn("gm:处理设置灵童养成类食草料等级,unrealDanLevel模板不存在")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		return
	}
	...
	manager.GmSetLingTongDevCulLevel(classType, int32(unrealDanLevel))

	//同步属性
	lingtongdevlogic.LingTongDevPropertyChanged(pl, classType)

	scLingTongDevCulDan := pbutil.BuildSCLingTongDevCulDan(int32(classType), int32(unrealDanLevel), 0)
