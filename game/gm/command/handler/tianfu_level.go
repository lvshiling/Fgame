...
func init() {
	command.Register(gmcommandtypes.CommandTypeTianFuLevel, command.CommandHandlerFunc(handleTianFuLevel))

}

func handleTianFuLevel(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
