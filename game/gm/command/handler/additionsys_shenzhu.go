...
func init() {
	command.Register(gmcommandtypes.CommandTypeShenZhuLev, command.CommandHandlerFunc(handleAdditionSysShenZhuLev))
}

func handleAdditionSysShenZhuLev(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) < 3 {
	...
	if !slotId.Valid() {
		...
		log.WithFields(
			...
			}).Warn("additionsysGm:神铸位置,错误")
		return
	}

	tempTemplatet := additionsystemplate.GetAdditionSysTemplateService().GetShenZhuByArg(slotId, int32(lev))
	...
	flag := manager.GmSetShenZhuLev(sysType, slotId, tempTemplatet.Level)
	if !flag {
		log.WithFields(
			log.Fields{
				"id":  pl.GetId(),
