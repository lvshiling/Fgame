...
func init() {
	command.Register(gmcommandtypes.CommandTypeEquipBaoKuAttendTimes, command.CommandHandlerFunc(handleSetEquipBaoKuAttendTimes))
}

func handleSetEquipBaoKuAttendTimes(p scene.Player, c *command.Command) (err error) {
	pl := p.(player.Player)
	if len(c.Args) <= 1 {
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"num":   num,
				"error": err,
			}).Warn("gm:处理装备宝库参与次数任务,类型num不是数字")
		playerlogic.SendSystemMessage(pl, lang.GMFormatWrong)
		err = nil
		return
	}
	...
	manager.GMSetTotalAttendTimes(int32(num), baoKuTyp)
	return
}
