...
func init() {
	found.RegistFoundCheckHandler(foundtypes.FoundResourceTypeGodSiege, found.FoundCheckHandlerFunc(checkFoundBack))
}
...
func checkFoundBack(pl player.Player) bool {
	...
	timeTemp := activitytemplate.GetActivityTemplateService().GetActiveByType(activitytypes.ActivityTypeGodSiegeQiLin).GetOnDateTimeTemplate(preDay, openTime, mergeTime)
	if timeTemp == nil {
		return false
	}
	return true
