...
func init() {
	playerinventory.RegisterUseHandler(itemtypes.ItemTypeGoldEquipStrengthen, itemtypes.ItemGoldEquipStrengthenSubTypeQiangHuaShengShi, playerinventory.ItemUseHandleFunc(handlerGoldEquipQiangHuaUpstar))
}

//元神金装强化升星道具
// args:param1:1背包2身体；param2:强化升星位置
func handlerGoldEquipQiangHuaUpstar(pl player.Player, it *playerinventory.PlayerItemObject, num int32, chooseIndexList []int32, args string) (flag bool, err error) {
	itemId := it.ItemId
	...
	if positionType == 1 {
		flag, err = qiangHuaBagEquip(pl, itemId, int32(targetIndex))
	} else {
		// flag, err = qiangHuaBodyEquip(pl, itemId, int32(targetIndex))
		//新强化等级
		flag, err = qiangHuaBuWeiEquip(pl, itemId, int32(targetIndex))
	}

	return
}

func qiangHuaBagEquip(pl player.Player, itemId int32, targetIndex int32) (flag bool, err error) {
	...
	if targetIt == nil || targetIt.IsEmpty() {
		log.WithFields(
			...
			}).Warn("goldequip:强化升星升级失败,强化升星目标不存在")
	...
	if goldEquipTemplate == nil {
		log.WithFields(
			log.Fields{
				...
				"targetIndex":  targetIndex,
			}).Warn("goldequip:强化升星升级失败,强化升星模板不存在")
		playerlogic.SendSystemMessage(pl, lang.CommonArgumentInvalid)
		return
	}
	...
	if goldEquipTemplate.GoldeuipUpstarId == 0 {
		log.WithFields(
			log.Fields{
				"playerId":    pl.GetId(),
				"targetIndex": targetIndex,
			}).Warn("goldequip:强化升星升级失败,该金装无法被强化升星")
	...
	if equipProperty.UpstarLevel >= toUplevel {
		log.WithFields(
			log.Fields{
				"playerId":    pl.GetId(),
				"targetIndex": targetIndex,
				"curLevel":    equipProperty.UpstarLevel,
				"toUplevel":   toUplevel,
			}).Warn("goldequip:强化升星升级失败,当前元神装备强化升星等级已超过")
		playerlogic.SendSystemMessage(pl, lang.GoldEquipLevelToHigh)
		return
	}

	//成功
	flag = inventoryManager.UpdateGoldEquipUpstarUseItem(targetIndex, itemId)
	if !flag {
		panic(fmt.Errorf("goldequip: 强化升星升级应该成功"))
	}
...
func qiangHuaBodyEquip(pl player.Player, itemId int32, targetSlotId int32) (flag bool, err error) {
	...
	if !posType.Valid() {
		log.WithFields(
			log.Fields{
				...
				"error":        err,
			}).Warn("goldequip:处理装备槽金装强化升星,位置错误")
		playerlogic.SendSystemMessage(pl, lang.CommonArgumentInvalid)
		return
	}
	...
	flag = equipBag.UpdateGoldEquipUpstarUseItem(posType, itemId)
	if !flag {
...
func qiangHuaBuWeiEquip(pl player.Player, itemId int32, targetSlotId int32) (flag bool, err error) {
	posType := inventorytypes.BodyPositionType(targetSlotId)
	if !posType.Valid() {
		log.WithFields(
			log.Fields{
	...
	if targetIt == nil || targetIt.IsEmpty() {
		log.WithFields(
			log.Fields{
				"playerId":     pl.GetId(),
				"targetSlotId": targetSlotId,
			}).Warn("goldequip:新强化升级失败,强化目标不存在")
		playerlogic.SendSystemMessage(pl, lang.InventoryItemNoExist)
		return
	}
	...
	itemTemplate := item.GetItemService().GetItem(int(itemId))
	toUplevel := itemTemplate.TypeFlag1
	if targetIt.GetNewStLevel() >= toUplevel {
		log.WithFields(
			log.Fields{
				"playerId":     pl.GetId(),
				"itemId":       itemId,
				"targetSlotId": targetSlotId,
				"toUplevel":    toUplevel,
			}).Warn("goldequip:新强化升级失败,当前元神装备强化等级已超过")
		playerlogic.SendSystemMessage(pl, lang.GoldEquipLevelToHigh)
		return
	}

	toUpTemp := goldequiptemplate.GetGoldEquipTemplateService().GetGoldEquipStrengthenBuWeiTemplate(posType, toUplevel)
	if toUpTemp == nil {
		log.WithFields(
			log.Fields{
				"playerId":     pl.GetId(),
	...
	flag = equipBag.UpdateGoldEquipStrengthBuWeiUseItem(posType, itemId)
	if !flag {
	...
	scMsg := pbutil.BuildSCGoldEquipUseItemWithGrowUp(itemId, posType, targetIt.GetNewStLevel())
	pl.SendMsg(scMsg)
	flag = true
	return
}
