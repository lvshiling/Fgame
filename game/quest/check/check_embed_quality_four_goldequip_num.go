...
func init() {
	quest.RegisterCheck(questtypes.QuestSubTypeEmbedQualityFourGoldEquipNum, quest.CheckHandlerFunc(handleEmbedQualityFourGoldEquipNum))
}

//check 穿戴元神金装的数量
func handleEmbedQualityFourGoldEquipNum(pl player.Player, questTemplate *gametemplate.QuestTemplate) (err error) {
	...
	for demandId, _ := range questDemandMap {
		...
		for _, slot := range slotList {
			...
			if itemTemp.Quality < int32(itemtypes.ItemQualityTypeOrange) {
				continue
			}
