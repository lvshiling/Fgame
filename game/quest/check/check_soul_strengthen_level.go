...
func init() {
	quest.RegisterCheck(questtypes.QuestSubTypeSoulStrengthenLevel, quest.CheckHandlerFunc(handleSoulStrengthenLevel))
}

//check 指定帝魂强化等级为X级
func handleSoulStrengthenLevel(pl player.Player, questTemplate *gametemplate.QuestTemplate) (err error) {
	log.Debug("quest:处理指定帝魂强化等级为X级")
	...
	for soulTag, _ := range questDemandMap {
		...
		level := soulInfo.StrengthenLevel
		if level == 0 {
			return
		}
