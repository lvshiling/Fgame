...
//强化帝魂X次
func init() {
	guaji.RegisterQuestGuaJi(questtypes.QuestSubTypeClickSoulStrengthenButton, guaji.QuestGuaJiFunc(clickSoulStrengthenLevel))
}

//强化帝魂X次
func clickSoulStrengthenLevel(p player.Player, questTemplate *gametemplate.QuestTemplate) bool {
	...
	for i := 0; i < int(needNum-num); i++ {
		quest.ClickHandle(p, clicktypes.ClickTypeSoul, clicktypes.ClickSubTypeSoulStrengthen)
	}
	return true
}
