...
func init() {
	guaji.RegisterQuestGuaJi(questtypes.QuestSubTypeClickSkillUpgradeButton, guaji.QuestGuaJiFunc(clickSkillUpgradeButton))
}

//直接升级
func clickSkillUpgradeButton(p player.Player, questTemplate *gametemplate.QuestTemplate) bool {
	...
	for i := 0; i < int(needNum-num); i++ {
		quest.ClickHandle(p, clicktypes.ClickTypeSkill, clicktypes.ClickSubTypeSkillUpgrade)
