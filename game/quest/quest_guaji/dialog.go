...
func init() {
	guaji.RegisterQuestGuaJi(questtypes.QuestSubTypeDialog, guaji.QuestGuaJiFunc(dialogQuestGuaJi))
}
...
func dialogQuestGuaJi(p player.Player, questTemplate *gametemplate.QuestTemplate) bool {
	...
	for npcId, _ := range demandMap {
		...
		if currentScene != nil && currentScene.MapId() == sceneTemplate.SceneID {
			if coreutils.Distance(p.GetPosition(), sceneTemplate.GetPos()) <= float64(collectDistance) {
				questlogic.IncreaseQuestData(p, questtypes.QuestSubTypeDialog, npcId, 1)
				return true
			}
		}
