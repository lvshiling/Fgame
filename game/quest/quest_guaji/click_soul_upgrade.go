...
//魂技升级X次
func init() {
	guaji.RegisterQuestGuaJi(questtypes.QuestSubTypeClickSoulUpgradeButton, guaji.QuestGuaJiFunc(clickSoulUpgradeButton))
}

//魂技升级X次
func clickSoulUpgradeButton(p player.Player, questTemplate *gametemplate.QuestTemplate) bool {
	...
	for i := 0; i < int(needNum-num); i++ {
		quest.ClickHandle(p, clicktypes.ClickTypeSoul, clicktypes.ClickSubTypeSoulUpgrade)
	}
	return true
}
