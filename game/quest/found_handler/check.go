...
func init() {
	found.RegistFoundCheckHandler(foundtypes.FoundResourceTypeAllianceDaily, found.FoundCheckHandlerFunc(checkFoundBack))
}
...
func checkFoundBack(pl player.Player) bool {
	...
	timeTemp := activitytemplate.GetActivityTemplateService().GetActiveByType(activitytypes.ActivityTypeLianYu).GetOnDateTimeTemplate(preDay, openTime, mergeTime)
	if timeTemp == nil {
		return false
	}
