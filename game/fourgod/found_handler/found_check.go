...
func init() {
	found.RegistFoundCheckHandler(foundtypes.FoundResourceTypeFourGod, found.FoundCheckHandlerFunc(checkFoundBack))
}
...
func checkFoundBack(pl player.Player) bool {
	...
	timeTemp := activitytemplate.GetActivityTemplateService().GetActiveByType(activitytypes.ActivityTypeFourGod).GetOnDateTimeTemplate(preDay, openTime, mergeTime)
	if timeTemp == nil {
		return false
	}
	return true
