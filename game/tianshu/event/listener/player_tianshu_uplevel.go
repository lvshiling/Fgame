...
//玩家天书升级
func tianshuUplevel(target event.EventTarget, data event.EventData) (err error) {
	...
	pl.UplevelTianShu(typ, rate)

	return
}

func init() {
	gameevent.AddEventListener(tianshueventtypes.EventTypePlayerTianShuUplevel, event.EventListenerFunc(tianshuUplevel))
}
