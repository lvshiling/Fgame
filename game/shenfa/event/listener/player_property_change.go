...
func playerPropertyChange(target event.EventTarget, data event.EventData) (err error) {
	...
	case playerpropertytypes.PlayerPropertyEffectorTypeShenfa:
	default:
		return
	}
	pl := target.(player.Player)
	manager := pl.GetPlayerDataManager(playertypes.PlayerShenfaDataManagerType).(*playershenfa.PlayerShenfaDataManager)
	propertyManager := pl.GetPlayerDataManager(types.PlayerPropertyDataManagerType).(*playerproperty.PlayerPropertyDataManager)
	power := propertyManager.GetModuleForce(playerpropertytypes.PlayerPropertyEffectorTypeShenfa)
	manager.ShenfaPower(power)
	return
}
