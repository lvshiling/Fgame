...
func init() {
	scene.RegisterCheckEnterHandler(scenetypes.SceneTypeTower, scene.CheckEnterSceneHandlerFunc(checkEnterScene))
}
...
func checkEnterScene(spl scene.Player, s scene.Scene, pos coretypes.Position, enterType scenetypes.SceneEnterType) (flag bool, enterPos coretypes.Position) {
	...
	if !pl.IsFuncOpen(funcopentypes.FuncOpenTypeDaBaoTower) {
		log.WithFields(
			log.Fields{
				"playerId": pl.GetId(),
