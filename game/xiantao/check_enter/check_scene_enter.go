...
func init() {
	scene.RegisterCheckEnterHandler(scenetypes.SceneTypeXianTaoDaHui, scene.CheckEnterSceneHandlerFunc(checkEnterScene))
}
...
func checkEnterScene(spl scene.Player, s scene.Scene, pos coretypes.Position, enterType scenetypes.SceneEnterType) (flag bool, enterPos coretypes.Position) {
	...
	if !pl.IsFuncOpen(funcopentypes.FuncOpenTypeXianTaoDaHui) {
		log.WithFields(
			log.Fields{
				"playerId": pl.GetId(),
