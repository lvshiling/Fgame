...
func init() {
	guaji.RegisterGuaJiEnterCheckHandler(guajitypes.GuaJiTypeXianFuSilver, guaji.GuaJiEnterCheckHandlerFunc(xianFuSilverEnterCheck))
}

func xianFuSilverEnterCheck(pl player.Player) bool {
	if !pl.IsFuncOpen(funcopentypes.FuncOpenTypeXinaFu) {
		return false
	}
	xianfuType := xianfutypes.XianfuTypeSilver
	...
	//判断是否要自动购买
	guaJiData := guaJiManager.GetGuaJiType(guajitypes.GuaJiTypeXianFuSilver)
	if guaJiData.GetOptionValue(guajitypes.GuaJiTypeSilverFuBenOptionTypeAutoBuy) == 0 {
		return false
	}
