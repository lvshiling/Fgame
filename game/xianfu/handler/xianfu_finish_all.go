...
func init() {
	processor.Register(codec.MessageType(uipb.MessageType_CS_XIANFU_FINISH_ALL_TYPE), dispatch.HandlerFunc(handlerXianfuFinishAll))
}

//秘境仙府一键完成请求
func handlerXianfuFinishAll(s session.Session, msg interface{}) (err error) {
	log.Debug("xianfu:处理秘境仙府一键完成请求")

	pl := gamesession.SessionInContext(s.Context()).Player()
	tpl := pl.(player.Player)
	csXianfuFinishAll := msg.(*uipb.CSXianfuFinishAll)
	typ := csXianfuFinishAll.GetXianfuType()
	//验证参数
	...
	log.WithFields(
		log.Fields{
			"playerId":   tpl.GetId(),
			"xianfuType": xianfuType,
		}).Debug("xianfu：处理秘境仙府一键完成请求完成")

	return
}

//仙府一键完成逻辑
func xianfuFinishAll(pl player.Player, xianfuType xianfutypes.XianfuType) (err error) {
	...
	//判断元宝是否足够
	if !propertyManager.HasEnoughGold(int64(finishAllNeedGold), true) {
		log.WithFields(
			log.Fields{
				"playerId":   pl.GetId(),
				"xianfuType": xianfuType,
			}).Warn("xianfu:秘境仙府一键完成请求，当前元宝不足，无法升级")
	...
	//消耗元宝
	goldReason := commonlog.GoldLogReasonXianfuFinishAllUse
	...
	//增加物品
	itemReason := commonlog.InventoryLogReasonXianfuSaodangRewards
	...
	//获取一键完成固定资源
	reasonGold := commonlog.GoldLogReasonXianfuFinishAllRew
	reasonSilver := commonlog.SilverLogReasonXianfuFinishAllRew
	reasonLevel := commonlog.LevelLogReasonXianfuFinishAllRew
	...
	scXianfuSaoDang := pbutil.BuildSCXianfuFinishAll(xianfuId, xianfuType, totalRewData, showItemList)
