...
//坐骑幻化配置
type XianTiHuanHuaTemplate struct {
	*XianTiHuanHuaTemplateVO
	useItemMap         map[int32]int32 //进阶物品
	unrealItemTemplate *ItemTemplate
}

func (mhht *XianTiHuanHuaTemplate) TemplateId() int {
	return mhht.Id
}

func (mhht *XianTiHuanHuaTemplate) GetUseItemTemplate() map[int32]int32 {
	return mhht.useItemMap
}

func (mhht *XianTiHuanHuaTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (mhht *XianTiHuanHuaTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(mhht.FileName(), mhht.TemplateId(), err)
			return
	...
	if mhht.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(mhht.NextId), (*XianTiHuanHuaTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", mhht.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		huanHuaTemplate := to.(*XianTiHuanHuaTemplate)
	...
	if mhht.unrealItemTemplate != nil {
		if mhht.unrealItemTemplate.GetItemSubType() != itemtypes.ItemXianTiSubTypeUnreal {
			err = fmt.Errorf("[%d] invalid", mhht.UseItem)
...
func (mhht *XianTiHuanHuaTemplate) PatchAfterCheck() {

}
func (mhht *XianTiHuanHuaTemplate) FileName() string {
	return "tb_xianti_huanhua.json"
}

func init() {
	template.Register((*XianTiHuanHuaTemplate)(nil))
}
