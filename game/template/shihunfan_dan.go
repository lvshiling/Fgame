...
//噬魂幡丹配置
type ShiHunFanDanTemplate struct {
	*ShiHunFanDanTemplateVO
	useItemMap            map[int32]int32 //进阶物品
	shiHunFanItemTemplate *ItemTemplate
}

func (t *ShiHunFanDanTemplate) TemplateId() int {
	return t.Id
}

func (t *ShiHunFanDanTemplate) GetUseItemTemplate() map[int32]int32 {
	return t.useItemMap
}

func (t *ShiHunFanDanTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
	...
	if t.UseItem != 0 {
		...
		t.shiHunFanItemTemplate = useItemTemplate.(*ItemTemplate)
...
func (t *ShiHunFanDanTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	if t.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(t.NextId), (*ShiHunFanDanTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		danTemplate := to.(*ShiHunFanDanTemplate)
		diffLevel := danTemplate.Level - t.Level
		if diffLevel != 1 {
			err = fmt.Errorf("[%d] invalid", t.Level)
	...
	if t.shiHunFanItemTemplate != nil {
		if t.shiHunFanItemTemplate.GetItemSubType() != itemtypes.ItemXueMoSubTypePeiYang {
			err = fmt.Errorf("[%d] invalid", t.UseItem)
...
func (t *ShiHunFanDanTemplate) PatchAfterCheck() {

}
func (t *ShiHunFanDanTemplate) FileName() string {
	return "tb_shihunfan_peiyang.json"
}

func init() {
	template.Register((*ShiHunFanDanTemplate)(nil))
}
