...
//灵童时装升星配置
type LingTongFashionUpstarTemplate struct {
	*LingTongFashionUpstarTemplateVO
	needItemMap                       map[int32]int32 //升星需要物品
	nextLingTongFashionUpstarTemplate *LingTongFashionUpstarTemplate
...
func (t *LingTongFashionUpstarTemplate) TemplateId() int {
	return t.Id
}

func (t *LingTongFashionUpstarTemplate) GetNeedItemMap() map[int32]int32 {

	return t.needItemMap
}

func (t *LingTongFashionUpstarTemplate) GetBattlePropertyMap() map[propertytypes.BattlePropertyType]int64 {
	return t.battlePropertyMap
}

func (t *LingTongFashionUpstarTemplate) GetLingTongBattlePropertyMap() map[propertytypes.BattlePropertyType]int64 {
	return t.lingTongBattlePropertyMap
}

func (t *LingTongFashionUpstarTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
	...
	//验证 next_id
	if t.NextId != 0 {
		to := template.GetTemplateService().Get(int(t.NextId), (*LingTongFashionUpstarTemplate)(nil))
		if to != nil {
			nextTemplate := to.(*LingTongFashionUpstarTemplate)
			diffLevel := nextTemplate.Level - t.Level
			if diffLevel != 1 {
				err = fmt.Errorf("[%d] invalid", nextTemplate.Level)
				return template.NewTemplateFieldError("Level", err)
			}
			t.nextLingTongFashionUpstarTemplate = nextTemplate
		}
	}
...
func (t *LingTongFashionUpstarTemplate) PatchAfterCheck() {

}

func (t *LingTongFashionUpstarTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	for itemId, _ := range t.needItemMap {
		...
		if itemTemplate.GetItemType() != itemtypes.ItemTypeLingTongFashion {
...
func (t *LingTongFashionUpstarTemplate) FileName() string {
	return "tb_lingtong_fashion_upstar.json"
}

func init() {
	template.Register((*LingTongFashionUpstarTemplate)(nil))
}
