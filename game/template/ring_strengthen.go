...
// 特戒进阶配置
type RingStrengthenTemplate struct {
	*RingStrengthenTemplateVO
	nextTemp    *RingStrengthenTemplate
	needItemMap map[int32]int32
}

func (t *RingStrengthenTemplate) TemplateId() int {
	return t.Id
}

func (t *RingStrengthenTemplate) GetNeedItemMap() map[int32]int32 {
	return t.needItemMap
}

func (t *RingStrengthenTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (t *RingStrengthenTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	// 验证 next_id
	if t.NextId != 0 {
		to := template.GetTemplateService().Get(int(t.NextId), (*RingStrengthenTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}
		t.nextTemp = to.(*RingStrengthenTemplate)
...
func (t *RingStrengthenTemplate) PatchAfterCheck() {
}

func (t *RingStrengthenTemplate) FileName() string {
	return "tb_tejie_strengthen.json"
}

func init() {
	template.Register((*RingStrengthenTemplate)(nil))
}
