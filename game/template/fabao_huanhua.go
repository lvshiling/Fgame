...
//法宝幻化配置
type FaBaoHuanHuaTemplate struct {
	*FaBaoHuanHuaTemplateVO
	useItemMap         map[int32]int32 //进阶物品
	unrealItemTemplate *ItemTemplate
}

func (whht *FaBaoHuanHuaTemplate) TemplateId() int {
	return whht.Id
}

func (whht *FaBaoHuanHuaTemplate) GetUseItemTemplate() map[int32]int32 {
	return whht.useItemMap
}

func (whht *FaBaoHuanHuaTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (whht *FaBaoHuanHuaTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(whht.FileName(), whht.TemplateId(), err)
			return
	...
	if whht.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(whht.NextId), (*FaBaoHuanHuaTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", whht.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		huanHuaTemplate := to.(*FaBaoHuanHuaTemplate)
	...
	if whht.unrealItemTemplate != nil {
		if whht.unrealItemTemplate.GetItemSubType() != itemtypes.ItemFaBaoSubTypeUnreal {
			err = fmt.Errorf("[%d] invalid", whht.UseItem)
...
func (whht *FaBaoHuanHuaTemplate) PatchAfterCheck() {

}
func (whht *FaBaoHuanHuaTemplate) FileName() string {
	return "tb_fabao_huanhua.json"
}

func init() {
	template.Register((*FaBaoHuanHuaTemplate)(nil))
}
