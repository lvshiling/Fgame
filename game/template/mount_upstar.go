...
//坐骑皮肤升星配置
type MountUpstarTemplate struct {
	*MountUpstarTemplateVO
	needItemMap             map[int32]int32 //升星需要物品
	nextMountUpstarTemplate *MountUpstarTemplate
	useItemTemplate         *ItemTemplate
}

func (mut *MountUpstarTemplate) TemplateId() int {
	return mut.Id
}

func (mut *MountUpstarTemplate) GetNeedItemMap() map[int32]int32 {
	return mut.needItemMap
}

func (mut *MountUpstarTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
	...
	//验证 next_id
	if mut.NextId != 0 {
		to := template.GetTemplateService().Get(int(mut.NextId), (*MountUpstarTemplate)(nil))
		if to != nil {
			nextTemplate := to.(*MountUpstarTemplate)
			diffLevel := nextTemplate.Level - mut.Level
			if diffLevel != 1 {
				err = fmt.Errorf("[%d] invalid", nextTemplate.Level)
				return template.NewTemplateFieldError("Level", err)
			}
			mut.nextMountUpstarTemplate = nextTemplate
		}
	}
...
func (mut *MountUpstarTemplate) PatchAfterCheck() {

}

func (mut *MountUpstarTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(mut.FileName(), mut.TemplateId(), err)
			return
	...
	//验证 equip_percent
	err = validator.RangeValidate(float64(mut.MountPercent), float64(0), true, float64(common.MAX_RATE), true)
	if err != nil {
		err = fmt.Errorf("[%d] invalid", mut.MountPercent)
	...
	if mut.useItemTemplate != nil {
		if mut.useItemTemplate.GetItemType() != itemtypes.ItemTypePet {
...
func (mut *MountUpstarTemplate) FileName() string {
	return "tb_mount_upstar.json"
}

func init() {
	template.Register((*MountUpstarTemplate)(nil))
}
