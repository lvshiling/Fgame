...
//领域幻化配置
type LingyuHuanHuaTemplate struct {
	*LingyuHuanHuaTemplateVO
	useItemMap         map[int32]int32 //进阶物品
	unrealItemTemplate *ItemTemplate
}

func (t *LingyuHuanHuaTemplate) TemplateId() int {
	return t.Id
}

func (t *LingyuHuanHuaTemplate) GetUseItemTemplate() map[int32]int32 {
	return t.useItemMap
}

func (t *LingyuHuanHuaTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (t *LingyuHuanHuaTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	if t.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(t.NextId), (*LingyuHuanHuaTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		huanHuaTemplate := to.(*LingyuHuanHuaTemplate)
	...
	if t.unrealItemTemplate != nil {
		if t.unrealItemTemplate.GetItemSubType() != itemtypes.ItemLingyuSubTypeUnreal {
			err = fmt.Errorf("[%d] invalid", t.UseItem)
...
func (t *LingyuHuanHuaTemplate) PatchAfterCheck() {

}
func (t *LingyuHuanHuaTemplate) FileName() string {
	return "tb_field_huanhua.json"
}

func init() {
	template.Register((*LingyuHuanHuaTemplate)(nil))
}
