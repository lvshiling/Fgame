...
//血盾培养配置
type BloodPeiYangTemplate struct {
	*BloodPeiYangTemplateVO
	useItemMap        map[int32]int32 //进阶物品
	battlePropertyMap map[propertytypes.BattlePropertyType]int64
}

func (t *BloodPeiYangTemplate) TemplateId() int {
	return t.Id
}

func (t *BloodPeiYangTemplate) GetUseItemTemplate() map[int32]int32 {
	return t.useItemMap
}

func (t *BloodPeiYangTemplate) GetBattleProperty() map[propertytypes.BattlePropertyType]int64 {
	return t.battlePropertyMap
}

func (t *BloodPeiYangTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (t *BloodPeiYangTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	if t.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(t.NextId), (*BloodPeiYangTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		bloodShieldTemplate := to.(*BloodPeiYangTemplate)
	...
	//验证物品
	for itemId, _ := range t.useItemMap {
		...
		if itemTemplate.GetItemSubType() != itemtypes.ItemXueDunSubTypeDebris {
			err = fmt.Errorf("[%d] invalid", t.UseItem)
			return template.NewTemplateFieldError("UseItem", err)
		}
	}
...
func (t *BloodPeiYangTemplate) PatchAfterCheck() {

}
func (t *BloodPeiYangTemplate) FileName() string {
	return "tb_blood_peiyang.json"
}

func init() {
	template.Register((*BloodPeiYangTemplate)(nil))
}
