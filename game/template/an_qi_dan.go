...
//暗器丹配置
type AnqiDanTemplate struct {
	*AnqiDanTemplateVO
	useItemMap       map[int32]int32 //进阶物品
	anQiItemTemplate *ItemTemplate
}

func (t *AnqiDanTemplate) TemplateId() int {
	return t.Id
}

func (t *AnqiDanTemplate) GetUseItemTemplate() map[int32]int32 {
	return t.useItemMap
}

func (t *AnqiDanTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
	...
	if t.UseItem != 0 {
		...
		t.anQiItemTemplate = useItemTemplate.(*ItemTemplate)
...
func (t *AnqiDanTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	if t.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(t.NextId), (*AnqiDanTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		danTemplate := to.(*AnqiDanTemplate)
	...
	if t.anQiItemTemplate != nil {
		if t.anQiItemTemplate.GetItemSubType() != itemtypes.ItemAnQiSubTypePeiYang {
			err = fmt.Errorf("[%d] invalid", t.UseItem)
...
func (t *AnqiDanTemplate) PatchAfterCheck() {

}
func (t *AnqiDanTemplate) FileName() string {
	return "tb_anqi_peiyang.json"
}

func init() {
	template.Register((*AnqiDanTemplate)(nil))
}
