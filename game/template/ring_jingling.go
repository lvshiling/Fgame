...
// 特戒进阶配置
type RingJingLingTemplate struct {
	*RingJingLingTemplateVO
	nextTemp    *RingJingLingTemplate
	needItemMap map[int32]int32
}

func (t *RingJingLingTemplate) TemplateId() int {
	return t.Id
}

func (t *RingJingLingTemplate) GetNeedItemMap() map[int32]int32 {
	return t.needItemMap
}

func (t *RingJingLingTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (t *RingJingLingTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	// 验证 next_id
	if t.NextId != 0 {
		to := template.GetTemplateService().Get(int(t.NextId), (*RingJingLingTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}
		t.nextTemp = to.(*RingJingLingTemplate)
...
func (t *RingJingLingTemplate) PatchAfterCheck() {
}

func (t *RingJingLingTemplate) FileName() string {
	return "tb_tejie_jingling.json"
}

func init() {
	template.Register((*RingJingLingTemplate)(nil))
}
