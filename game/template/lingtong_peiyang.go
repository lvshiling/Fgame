...
//灵童培养配置
type LingTongPeiYangTemplate struct {
	*LingTongPeiYangTemplateVO
	useItemMap                map[int32]int32 //培养物品
	culItemTemplate           *ItemTemplate
	nextTemplate              *LingTongPeiYangTemplate
...
func (t *LingTongPeiYangTemplate) TemplateId() int {
	return t.Id
}

func (t *LingTongPeiYangTemplate) GetUseItemTemplate() map[int32]int32 {
	return t.useItemMap
}

func (t *LingTongPeiYangTemplate) GetBattleProperty() map[propertytypes.BattlePropertyType]int64 {
	return t.battlePropertyMap
}

func (t *LingTongPeiYangTemplate) GetLingTongBattlePropertyMap() map[propertytypes.BattlePropertyType]int64 {
	return t.lingTongBattlePropertyMap
}

func (t *LingTongPeiYangTemplate) Patch() (err error) {
	defer func() {
		if err != nil {
...
func (t *LingTongPeiYangTemplate) Check() (err error) {
	defer func() {
		if err != nil {
			err = template.NewTemplateError(t.FileName(), t.TemplateId(), err)
			return
	...
	if t.NextId != 0 {
		...
		to := template.GetTemplateService().Get(int(t.NextId), (*LingTongPeiYangTemplate)(nil))
		if to == nil {
			err = fmt.Errorf("[%d] invalid", t.NextId)
			return template.NewTemplateFieldError("NextId", err)
		}

		t.nextTemplate = to.(*LingTongPeiYangTemplate)
	...
	if t.culItemTemplate != nil {
		if t.culItemTemplate.GetItemSubType() != itemtypes.ItemLingTongSubTypePeiYang {
			err = fmt.Errorf("[%d] invalid", t.UseItem)
...
func (t *LingTongPeiYangTemplate) PatchAfterCheck() {

}
func (t *LingTongPeiYangTemplate) FileName() string {
	return "tb_lingtong_peiyang.json"
}

func init() {
	template.Register((*LingTongPeiYangTemplate)(nil))
}
