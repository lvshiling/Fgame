...
func init() {
	scene.RegisterReliveHandler(scenetypes.SceneTypeZhenXi, scene.ReliveHandlerFunc(relive))
...
func relive(pl scene.Player, autoBuy bool) {
	...
	maxReliveTime := constant.GetConstantService().GetConstant(constanttypes.ConstantTypeZhenXiReliveTimes)
	...
	// manager.Relive(worldbosstypes.BossTypeZhenXi)
	// reliveTime = manager.GetReliveTime()
	// scPlayerZhenXiBossInfo := pbutil.BuildSCPlayerZhenXiBossInfo(pl, reliveTime)
