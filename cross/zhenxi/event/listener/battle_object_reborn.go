...
func battleObjectReborn(target event.EventTarget, data event.EventData) (err error) {
	...
	//校验怪物类型
	if n.GetBiologyTemplate().GetBiologyScriptType() != scenetypes.BiologyScriptTypeZhenXiBoss {
		return
	}

	scBroadcast := pbutil.BuildSCWorldBossInfoBroadcast(n, worldbosstypes.BossTypeZhenXi)
	...
	args := []int64{int64(chattypes.ChatLinkTypeOpenView), funcopentypes.FuncOpenTypeZhenXiBoss, bossId}
