...
func handleMoFangYouXiFengMoIOS(rw http.ResponseWriter, req *http.Request) {
	query := req.URL.Query()
	...
	if err != nil {
		log.WithFields(
			log.Fields{
				...
				"sign":    sign,
				"error":   err,
			}).Warn("charge:魔方游戏-封魔录ios充值请求，解析错误")
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:魔方游戏-封魔录ios充值请求,sdk配置为空")
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:魔方游戏-封魔录ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	if sign != getSign {
		log.WithFields(
			log.Fields{
				...
				"getSign":     getSign,
				"error":       err,
			}).Warn("charge:魔方游戏-封魔录ios充值请求,签名错误")
		rw.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if obj == nil {
		log.WithFields(
			log.Fields{
				...
				"sign":        sign,
				"error":       err,
			}).Warn("charge:魔方游戏-封魔录ios请求,订单不存在")
		rw.WriteHeader(http.StatusBadRequest)
		return
	}
	if !repeat {
...
func moFangYouXiFengMoSign(key string, userid string, server int32, money string, pay string, order string, timeInt int64) (sign string) {
