...
func Router(r *mux.Router) {
	sr := r.PathPrefix(wenjianPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleWenJianIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleWenJianAndroid))
}

func handleWenJianAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeWenJian
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:问剑安卓充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.WenJianConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:问剑安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := wenJianSign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeWenJian, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
