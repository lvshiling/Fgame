...
func handleLongYuIOS(rw http.ResponseWriter, req *http.Request) {
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:龙语ios充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	qiAConfig, ok := sdkConfig.(*sdksdk.LongYuConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:龙语ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func GetLongYuSign(reqForm *LongYuRequest, appKey string) (sign string) {
