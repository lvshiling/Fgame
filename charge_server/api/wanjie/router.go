...
func Router(r *mux.Router) {
	sr := r.PathPrefix(wanjiePath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleWanJieIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleWanJieAndroid))
}

func handleWanJieAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeWanJie
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:万界奇谭安卓充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.WanJieConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:万界奇谭安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := wanJieSign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeWanJie, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
