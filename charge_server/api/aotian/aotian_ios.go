...
func handleAoTianIOS(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:傲天ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func GetAoTianSign(reqForm *AoTianRequest, appKey string) (sign string) {
