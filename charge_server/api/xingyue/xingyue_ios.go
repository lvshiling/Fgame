...
func handleXingYueIOS(rw http.ResponseWriter, req *http.Request) {
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:星月ios充值请求,sdk配置为空")
		result := "服务器错误"
		rw.WriteHeader(http.StatusOK)
		rw.Write([]byte(result))
		return
	}
	boCaiConfig, ok := sdkConfig.(*sdksdk.XingYueConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:星月ios充值请求,sdk配置强制转换失败")
		result := "服务器错误"
		rw.WriteHeader(http.StatusOK)
		rw.Write([]byte(result))
		return
