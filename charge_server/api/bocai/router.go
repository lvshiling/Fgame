...
func Router(r *mux.Router) {
	sr := r.PathPrefix(boCaiPath).Subrouter()
	sr.Path("/android").Handler(http.HandlerFunc(handleBoCaiAndroid))
	sr.Path("/ios").Handler(http.HandlerFunc(handleBoCaiIOS))
}

type BoCaiForm struct {
...
func handleBoCaiAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:菠菜安卓充值请求,sdk配置强制转换失败")
		result := "服务器错误"
		rw.WriteHeader(http.StatusOK)
		rw.Write([]byte(result))
		return
	...
	log.WithFields(
		log.Fields{
			...
			"memo":         memo,
			"uid":          uid,
		}).Info("charge:菠菜安卓充值请求,成功")
}

func convertBoCaiDataToMap(data *BoCaiForm) (dataMap map[string]string) {
...
func GetSign(reqForm *BoCaiForm, payKey string) (sign string) {
