...
func Router(r *mux.Router) {
	sr := r.PathPrefix(xianfanPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleXianFanIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleXianFanAndroid))
}

func handleXianFanAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeXianFan
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:仙凡安卓充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.XianFanConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:仙凡安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := xianFanSign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeXianFan, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
