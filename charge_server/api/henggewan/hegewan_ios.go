...
func handleHengGeWanIOS(rw http.ResponseWriter, req *http.Request) {
	query := req.URL.Query()
	...
	sdkType := logintypes.SDKTypeHengGeWan
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:亨哥玩ios充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.HengGeWanConfig)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:亨哥玩ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := hengGeWanSign(chargeKey, userId, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeHengGeWan, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
...
func hengGeWanSign(key string, userid int64, server int32, money string, pay string, order string, timeInt int64) (sign string) {
	allStr := fmt.Sprintf("%d%d%s%s%s%d%s", userid, server, money, pay, order, timeInt, key)
