...
func Router(r *mux.Router) {
	sr := r.PathPrefix(jiulingPath).Subrouter()
	sr.Path("/android").Handler(http.HandlerFunc(handleJiuLingAndroid))
	sr.Path("/ios").Handler(http.HandlerFunc(handleJiuLingIOS))
}

func handleJiuLingAndroid(rw http.ResponseWriter, req *http.Request) {
	reqForm := &JiuLingRequest{}
	...
	log.WithFields(
		log.Fields{
			...
			"sign":     reqForm.Sign,
		}).Info("charge:九零安卓充值请求")

	sdkType := logintypes.SDKTypeJiuLing
	sdkConfig := sdk.GetSdkService().GetSdkConfig(sdkType)
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:九零安卓充值请求,sdk配置为空")
		result := "error"
		rw.Write([]byte(result))
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:九零安卓充值请求,sdk配置强制转换失败")
		result := "error"
		rw.Write([]byte(result))
		rw.WriteHeader(http.StatusInternalServerError)
		return
	...
	//TODO 验证签名
	getSign := GetJiuLingSign(reqForm, appKey)
	...
	obj, repeat, err := chargeService.OrderPay(reqForm.Attache, reqForm.OrderId, logintypes.SDKTypeJiuLing, reqForm.Amount, reqForm.RoleId, now)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":       req.RemoteAddr,
