...
func Router(r *mux.Router) {
	sr := r.PathPrefix(shunYouPath).Subrouter()
	sr.Path("/android").Handler(http.HandlerFunc(handleShunYouAndroid))
	sr.Path("/ios").Handler(http.HandlerFunc(handleShunYouIOS))
}

type ShunYouForm struct {
	State     int32        `form:"state" json:"state"`
	SdkUserId string       `form:"sdkUserID" json:"sdkUserID"`
	Data      *ShunYouData `form:"data" json:"data"`
}

type ShunYouData struct {
...
func handleShunYouAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	data := form.Data
	sdkType := logintypes.SDKTypeShunYou
	log.WithFields(
		log.Fields{
			"ip":        req.RemoteAddr,
			"state":     state,
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:顺游安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}

	devicePlatformType := logintypes.DevicePlatformTypeAndroid
	secretKey := shunYouConfig.GetSecretKey(devicePlatformType)
	publicKey := shunYouConfig.GetPublicKey(devicePlatformType)
	...
	//TODO 验证签名
	sign := data.Sign
	dataMap := convertShunYouDataToMap(data)
	originalData := GetShunYouOriginalData(dataMap, secretKey)
	...
	log.WithFields(
		log.Fields{
			...
			"sign":        sign,
		}).Info("charge:顺游安卓充值请求,成功")
}

func convertShunYouDataToMap(data *ShunYouData) (dataMap map[string]string) {
...
func GetShunYouOriginalData(dataStrMap map[string]string, secretKey string) (sign string) {
