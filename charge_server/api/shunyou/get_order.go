...
func init() {
	charge.RegisterSDKOrderHandler(logintypes.SDKTypeShunYou, charge.GetSDKOrderHandlerFunc(GetSDKOrder))
}
...
func GetSDKOrder(devicePlatformType logintypes.DevicePlatformType, platformUserId string, productId int32, money int32, roleId int64, roleName string, serverId int32, orderId string) (notifyUrl string, sdkOrderId string, extension string, flag bool) {
	...
	sdkConfig := sdk.GetSdkService().GetSdkConfig(logintypes.SDKTypeShunYou)
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
	...
	if !ok {
		log.WithFields(
			log.Fields{
				...
				"serverId":       serverId,
				"orderId":        orderId,
			}).Warn("charge:顺游下单,sdk配置强制转换失败")
		return
	}

	notifyUrl = shunYouConfig.GetNotifyUrl(devicePlatformType)
	appSecret := shunYouConfig.GetSecretKey(devicePlatformType)
	privateKey := shunYouConfig.GetPrivateKey(devicePlatformType)
	...
	if statusInt != 1 {
		log.WithFields(
			log.Fields{
				...
				"data":   sdkResult.Data,
			}).Warn("charge:获取订单号顺游手游,获取订单号验证失败")
		return
	}
