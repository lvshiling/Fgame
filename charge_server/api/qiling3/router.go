...
func Router(r *mux.Router) {
	sr := r.PathPrefix(qiling3Path).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleQiLing3IOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleQiLing3Android))
}

func handleQiLing3Android(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeQiLing3
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:启灵3安卓充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.QiLing3Config)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:启灵3安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := qiLing3Sign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeQiLing3, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
