...
func Router(r *mux.Router) {
	sr := r.PathPrefix(chenxi2Path).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleChenXi2IOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleChenXi2Android))
}

func handleChenXi2Android(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeChenXi2
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:晨曦2安卓充值请求,sdk配置为空")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.ChenXi2Config)
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:晨曦2安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := chenXi2Sign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeChenXi2, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
