...
func handleChenXi2IOS(rw http.ResponseWriter, req *http.Request) {
	query := req.URL.Query()
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:晨曦2ios充值请求,sdk配置为空")
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:晨曦2ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func chenXi2Sign(key string, userid string, server int32, money string, pay string, order string, timeInt int64) (sign string) {
