...
func Router(r *mux.Router) {
	sr := r.PathPrefix(disuiPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleDiSuiIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleDiSuiAndroid))
}

func handleDiSuiAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:地随世渊安卓充值请求,sdk配置为空")
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:地随世渊安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
