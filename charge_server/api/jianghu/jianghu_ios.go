...
func handleJiangHuIOS(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:江湖ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func GetJiangHuSign(reqForm *JiangHuRequest, appKey string) (sign string) {
