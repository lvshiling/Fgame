...
func Router(r *mux.Router) {
	sr := r.PathPrefix(zhengFuPath).Subrouter()
	sr.Path("/android").Handler(http.HandlerFunc(handleZhengFuAndroid))
	sr.Path("/ios").Handler(http.HandlerFunc(handleZhengFuIOS))
	// sr.Path("/get").Handler(http.HandlerFunc(handleGetZhengFuOrder))
}

type ZhengFuForm struct {
	State     int32        `form:"state" json:"state"`
	SdkUserId string       `form:"sdkUserID" json:"sdkUserID"`
	Data      *ZhengFuData `form:"data" json:"data"`
}

type ZhengFuData struct {
...
func handleZhengFuAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:征服安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	log.WithFields(
		log.Fields{
			...
			"sign":        sign,
		}).Info("charge:征服安卓充值请求,成功")
}

func convertZhengFuDataToMap(data *ZhengFuData) (dataMap map[string]string) {
...
func GetZhengFuOriginalData(dataStrMap map[string]string, secretKey string) (sign string) {
