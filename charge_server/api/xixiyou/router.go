...
func Router(r *mux.Router) {
	sr := r.PathPrefix(xixiyouPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleXiXiYouIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleXiXiYouAndroid))
}

func handleXiXiYouAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	sdkType := logintypes.SDKTypeXiXiYou
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:嘻嘻游安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := xiXiYouSign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeXiXiYou, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
