...
func Router(r *mux.Router) {
	sr := r.PathPrefix(bajiPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleBaJiIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleBaJiAndroid))
}

func handleBaJiAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:吧唧安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
