...
func handleXinFengIOS(rw http.ResponseWriter, req *http.Request) {
	query := req.URL.Query()
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:新蜂ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func xinFengSign(cpOrderIdStr, orderIdStr, appId, uid string, amount int32, receiveTime int64, key string) (sign string) {
