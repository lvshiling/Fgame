...
const (
	tianshenPath = "/tianshen"
)

func Router(r *mux.Router) {
	sr := r.PathPrefix(tianshenPath).Subrouter()
	sr.Path("/ios").Handler(http.HandlerFunc(handleTianShenIOS))
	sr.Path("/android").Handler(http.HandlerFunc(handleTianShenAndroid))
}

func handleTianShenAndroid(rw http.ResponseWriter, req *http.Request) {
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:天神决游安卓充值请求,sdk配置为空")
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip":    req.RemoteAddr,
				"error": err,
			}).Warn("charge:天神决游安卓充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
	...
	//TODO 验证签名
	getSign := tianShenSign(chargeKey, userIdStr, server, moneyStr, pay, order, receiveTime)
	if sign != getSign {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
	...
	obj, repeat, err := chargeService.OrderPay(order, pay, logintypes.SDKTypeTianShen, money, userIdStr, receiveTime)
	if err != nil {
		log.WithFields(
			log.Fields{
				"ip":          req.RemoteAddr,
