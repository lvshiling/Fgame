...
func handleJuDuIOS(rw http.ResponseWriter, req *http.Request) {
	reqForm := &JuDuRequest{}
	...
	if !ok {
		log.WithFields(
			log.Fields{
				"ip": req.RemoteAddr,
			}).Warn("charge:巨都ios充值请求,sdk配置强制转换失败")
		rw.WriteHeader(http.StatusInternalServerError)
		return
	}
...
func GetJuDuSign(reqForm *JuDuRequest, appKey string) (sign string) {
