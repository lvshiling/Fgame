...
func init() {
	login.RegisterLogin(types.SDKTypeDiSui, login.LoginHandlerFunc(handleDiSui))
}

func handleDiSui(devicePlatformType types.DevicePlatformType, msg interface{}) (flag bool, returnPlatform int32, userId string, err error) {
	csAccountLogin := msg.(*uipb.CSAccountLogin)

	xingTing := csAccountLogin.GetDiSui()
	if xingTing == nil {
		log.WithFields(
			log.Fields{}).Warn("login:登陆地随世渊,数据是空的")
		return
	}
	...
	if sdkConfig == nil {
		log.WithFields(
			log.Fields{}).Warn("login:登陆地随世渊,sdk配置为空")
		return
	}
	hengGeWanConfig, ok := sdkConfig.(*sdksdk.DiSuiConfig)
	if !ok {
		log.WithFields(
			log.Fields{}).Warn("login:登陆地随世渊,sdk配置强制转换失败")
		return
	}
	...
	//验证签名正确性
	getSign := DiSuiSign(key, tmsg, userId, timeStr)
	if getSign != sign {
		log.WithFields(
			log.Fields{
				"key":     key,
	...
	log.WithFields(
		log.Fields{
			...
			"sign":    sign,
			"tmsg":    tmsg,
		}).Info("login:登陆地随世渊,登陆成功")
	return
}

func DiSuiSign(key string, msg string, userid string, timeStr string) (sign string) {
