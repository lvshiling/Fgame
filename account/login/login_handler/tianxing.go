...
func init() {
	login.RegisterLogin(types.SDKTypeTianXing, login.LoginHandlerFunc(handleTianXing))
}

func handleTianXing(devicePlatformType types.DevicePlatformType, msg interface{}) (flag bool, returnPlatform int32, userId string, err error) {
	csAccountLogin := msg.(*uipb.CSAccountLogin)

	tianXing := csAccountLogin.GetTianXing()
	if tianXing == nil {
		log.WithFields(
	...
	tianXingConfig, ok := sdkConfig.(*sdksdk.TianXingConfig)
	if !ok {
		log.WithFields(
			log.Fields{}).Warn("login:登陆天行战歌,sdk配置强制转换失败")
		return
	}
	key := tianXingConfig.GetKey(devicePlatformType)
	timeStr := tianXing.GetTime()
	userId = tianXing.GetUserid()
	sign := tianXing.GetSign()
	tmsg := tianXing.GetMsg()
	//验证签名正确性
	getSign := TianXingSign(key, tmsg, userId, timeStr)
	if getSign != sign {
		log.WithFields(
			log.Fields{
				"key":     key,
...
func TianXingSign(key string, msg string, userid string, timeStr string) (sign string) {
