...
type Stats interface {
	Start()
	Stop()
	StatsRouter(r *mux.Router)
	HandleSessionOpenStats(s session.Session)
	HandleSessionRecvStats(s session.Session, msg []byte)
	HandleSessionSendStats(s session.Session, msg []byte)
	HandleSessionCloseStats(s session.Session) error
}
...
//-------------------统计--------------------------
func (g *stats) HandleSessionOpenStats(s session.Session) {
	atomic.AddInt64(&g.conn.CurrentConnections, 1)
	atomic.AddInt64(&g.conn.TotalConnections, 1)
}

//消息接收统计中间件
func (g *stats) HandleSessionRecvStats(s session.Session, msg []byte) {
	atomic.AddInt64(&g.msg.InMsgs, 1)
	atomic.AddInt64(&g.msg.InBytes, int64(len(msg)))
	return
}

//消息发送统计中间件
func (g *stats) HandleSessionSendStats(s session.Session, msg []byte) {
	msgLen := int64(len(msg))
	atomic.AddInt64(&g.msg.OutMsgs, 1)
	atomic.AddInt64(&g.msg.OutBytes, msgLen)
	return
}

//连接统计中间件 连接关闭
func (g *stats) HandleSessionCloseStats(s session.Session) error {
	atomic.AddInt64(&g.conn.CurrentConnections, -1)
	return nil
}

//统计http请求
func (g *stats) handleStats(rw http.ResponseWriter, req *http.Request) {
	sr := convertToStatsResult(g)
...
//连接统计http请求
func (g *stats) handleConnStats(rw http.ResponseWriter, req *http.Request) {
	err := httputils.WriteJSON(rw, http.StatusOK, g.conn)
	if err != nil {
		rw.WriteHeader(http.StatusInternalServerError)
	}
}
...
//接收消息和发送消息http统计
func (g *stats) handleMsgStats(rw http.ResponseWriter, req *http.Request) {
	err := httputils.WriteJSON(rw, http.StatusOK, g.msg)
	if err != nil {
		rw.WriteHeader(http.StatusInternalServerError)
	}
}

func (g *stats) StatsRouter(r *mux.Router) {
