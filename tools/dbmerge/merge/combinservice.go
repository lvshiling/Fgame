...
import (
	...
	exp "fgame/fgame/tools/dbmerge/export"
...
func (m *combinService) CombinService(p_fpid int64, p_fsid int, p_tpid int64, p_tsid int) error {
	if p_fpid != p_tpid {
		return fmt.Errorf("平台不一样不可以合服")
	}
	flag, err := m.ifMerge(int32(p_fpid), int32(p_fsid), int32(p_tsid))
	if err != nil {
		return err
	}
	if flag {
		return fmt.Errorf("已经合服过")
	}
	...
	//更改服务器ID
	log.Info("更改...")
	err = m.updateServerData(p_fsid, p_tsid)
	if err != nil {
		return err
	}
	//修改中心服数据
	log.Info("更新中心服数据...")
	//更新中心服合服记录
	flag, err = m.updateCenterMergeData(int32(p_fpid), int32(p_fsid), int32(p_tsid))
	if !flag {
		return fmt.Errorf("已经合服过")
	}
	if err != nil {
