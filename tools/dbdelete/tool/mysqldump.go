...
func DeleteTable(db *model.DBConfigInfo, p_table string, p_sql string, p_mysqlPath string) error {
	...
	deleteSql := fmt.Sprintf("delete from %s where %s", p_table, p_sql)
	arg = append(arg, fmt.Sprintf("--execute=%s", deleteSql))
