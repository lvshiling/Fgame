...
func start(c *cli.Context) {
	...
	tempGmDestAbsoluteDir, err := filepath.Abs(gmDestDir)
	if err != nil {
		log.WithFields(
			log.Fields{
				"error": err,
	...
	gmDestAbsoluteDir = tempGmDestAbsoluteDir
...
func convertGmClass(sc *Class) *Class {
	newSc := &Class{}
	newSc.Class = sc.Class
	newSc.Comment = sc.Comment
	newSc.File = sc.File
...
func GenerateGm(configFile string) (err error) {
	fmt.Println("genertae gm")
	sc, err := readClass(configFile)
	if err != nil {
		return
	}
	t, err := template.New("").Parse(gmStrTmp)
	if err != nil {
		return
	}
	buff := bytes.NewBufferString("")
	newSc := convertGmClass(sc)
	err = t.Execute(buff, newSc)
	if err != nil {
		return
	}
	...
	destPath := filepath.Join(gmDestAbsoluteDir, sc.File)
	err = ioutil.WriteFile(destPath, src, os.ModePerm)
	if err != nil {
		return
	}
