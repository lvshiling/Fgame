...
import (
	...
	wxcache "fgame/fgame/gzh/cache"
	wxapi "fgame/fgame/gzh/wxapi"
	wxrouter "fgame/fgame/gzh/wxapi/api"
...
type serverConfig struct {
	...
	Remote *wxapi.RemoteConfig `json:"remote"`
}
...
func start(ctx *cli.Context) {
	...
	if tsc.WxPay != nil {
		if tsc.WxPay.CertPath != "" {
			redbagcertpath, redcerterr := filepath.Abs(tsc.WxPay.CertPath)
			if redcerterr != nil {
				log.Println("RedBagCertpath error :", redcerterr)
			} else {
				tsc.WxPay.CertPath = redbagcertpath
			}
		} else {
	...
	remoteService := wxapi.NewRemoteService(sc.Remote)
	...
	n.UseFunc(setupRemoteServiceHandler(remoteService))
	n.UseHandler(router)
...
func setupRemoteServiceHandler(ls *wxapi.RemoteService) negroni.HandlerFunc {
	return negroni.HandlerFunc(func(rw http.ResponseWriter, req *http.Request, hf http.HandlerFunc) {
		ctx := req.Context()
		nctx := wxapi.WithRemoteService(ctx, ls)
		nreq := req.WithContext(nctx)
		hf(rw, nreq)
	})
}
